(() => {
  ...
  function formularioCalcinhaAtivo() {
    ...
    try { planejamentoVisivel = Boolean(planejamento && getComputedStyle(planejamento).display !== "none"); } catch (_) {}
  ...
  function toast(mensagem, tipo = "info") {
    ...
    if (!elemento) {
      ...
      Object.assign(elemento.style, {
        position: "fixed", right: "18px", bottom: "18px", zIndex: "100250", maxWidth: "460px",
        padding: "13px 16px", borderRadius: "12px", color: "#fff", font: "800 13px/1.45 Arial, sans-serif",
        boxShadow: "0 16px 40px rgba(0,0,0,.25)", opacity: "0", transform: "translateY(14px)",
        transition: ".2s ease", pointerEvents: "none"
      });
  ...
  async function firebase() {
    ...
    firebasePromise = Promise.all([
      ...
      let app = appModule.getApps()[0] || null;
      for (let tentativa = 0; !app && tentativa < 100; tentativa++) {
        await new Promise(resolve => setTimeout(resolve, 100));
        app = appModule.getApps()[0] || null;
      }
  ...
  async function buscarOrdensPorNumero(numeroOP) {
    ...
    const encontrados = await buscarPorIds([safeId(numeroTexto), `calcinha-${safeId(numeroTexto)}`, `op-${safeId(numeroTexto)}`]);
  ...
  async function buscarProdutoCalcinha(referencia) {
    ...
    const encontrados = new Map();
    try {
      const direto = await fb.getDoc(fb.doc(fb.db, "produtos", `calcinha-${safeId(referencia)}`));
  ...
  async function registrarLog(acao, alvoId, detalhes) {
    try {
      ...
      await fb.addDoc(fb.collection(fb.db, "logsAlteracoes"), {
        acao, tipoAlvo: "ordemProducao", alvoId: String(alvoId || ""), detalhes: String(detalhes || ""),
        usuarioUid: usuario.uid, usuarioEmail: usuario.email || "", criadoEm: fb.serverTimestamp()
      });
    } catch (erro) { console.warn("[Calcinha 137] Log não salvo.", erro); }
  }
  ...
  async function aplicarIdentidadeCalcinha(ordem, extras = {}) {
    ...
    const dados = {
      tipoPeca: "calcinha", tipoPecaPadrao: "calcinha", tipoPecaLabel: "Calcinha", setor: "calcinha",
      processoPlanejado: processo, faccaoPlanejada: faccao, planejamentoCalcinhaPendente: !(processo && faccao),
      possuiAlca: false, possuiBojo: false, possuiRenda: false,
      identidadeCalcinhaConfirmada: true, identidadeCalcinhaVersao: VERSION, reparoCalcinha137: true,
      atualizadoPor: usuario?.uid || "", atualizadoEm: fb.serverTimestamp(), ...extras
    };
    if (!PROCESSOS_CALCINHA.has(normalizar(dados.processoPlanejado))) {
  ...
  function atualizarTelaDepoisDoReparo() {
    setTimeout(() => {
      ...
      setTimeout(() => document.querySelector('.corponu-dual-tabs[data-page="ordens"] [data-type="calcinha"]')?.click(), 500);
    }, 250);
  }
  ...
  async function repararOP57864() {
    ...
    try {
      ...
      if (!ordens.length || ordens.find(ehCalcinha)) return;
      if (ordens.length !== 1) {
      ...
      const corrigida = await aplicarIdentidadeCalcinha(ordem, {
        ...
        processoPlanejado: "", faccaoPlanejada: "", planejamentoCalcinhaPendente: true
      });
      await registrarLog("ordem_calcinha_reparo_automatico_137", corrigida.id,
  ...
  async function salvarOrdemCalcinha(event) {
    ...
    try {
      ...
      if (!numeroOP || !referencia || !cor || !Number.isFinite(quantidade) || quantidade <= 0) throw new Error("Informe OP, referência, cor e quantidade válida.");
      ...
      if (incorretas.length > 1 || (encontradas.length && incorretas.length !== 1)) throw new Error(`A OP ${numeroOP} possui mais de um registro conflitante. Não alterei nada; confira antes de continuar.`);
      let documentoId = currentId || `calcinha-${safeId(numeroOP)}`;
      let ordemAntiga = {};
      if (incorretas.length === 1) {
        const antiga = incorretas[0];
        const confirmar = window.confirm(`A OP ${numeroOP} já existe classificada como Sutiã.\n\nVocê está cadastrando pela aba Calcinha. Deseja corrigir o registro existente para Calcinha usando os dados deste formulário?\n\nNenhuma OP nova será criada.`);
        if (!confirmar) return;
      ...
      const dados = {
        ...
        tipoPeca: "calcinha", tipoPecaPadrao: "calcinha", tipoPecaLabel: "Calcinha", setor: "calcinha",
        linhaCalcinha: ordemAntiga.linhaCalcinha || "", processoPlanejado: processo,
        faccaoPlanejada: processo ? faccao : "", planejamentoCalcinhaPendente: !planejamentoCompleto,
        possuiAlca: false, possuiBojo: false, possuiRenda: false,
        identidadeCalcinhaConfirmada: true, identidadeCalcinhaVersao: VERSION, reparoCalcinha137: Boolean(ordemAntiga.id),
        status: ordemAntiga.status || "aberta", atualizadoPor: usuario.uid, atualizadoEm: fb.serverTimestamp()
      };
      if (!ordemAntiga.criadoEm) { dados.criadoPor = usuario.uid; dados.criadoEm = fb.serverTimestamp(); }
      await fb.setDoc(fb.doc(fb.db, "ordensProducao", documentoId), dados, { merge: true });
      await registrarLog(ordemAntiga.id ? "ordem_corrigida_para_calcinha_137" : currentId ? "ordem_atualizada" : "ordem_criada",
        documentoId, `Calcinha | OP ${numeroOP} | Ref. ${referencia} | ${processo || "SERVIÇO A DEFINIR"} | ${faccao || "FACÇÃO A DEFINIR"}`);
      form.reset();
      ...
      toast(ordemAntiga.id ? `OP ${numeroOP} corrigida: agora pertence a Calcinha.` : `OP ${numeroOP} de calcinha salva corretamente.`, "success");
      atualizarTelaDepoisDoReparo();
    } catch (erro) {
  ...
  firebase().then(fb => {
    fb.onAuthStateChanged(fb.auth, user => { if (user) setTimeout(repararOP57864, 500); });
    if (fb.auth.currentUser) setTimeout(repararOP57864, 500);

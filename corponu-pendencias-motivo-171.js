(() => {
  ...
  function faltantesDoPagamento(pagamento) {
    ...
    if (Array.isArray(memoria) && memoria.length) {
      return memoria.map(texto).filter(Boolean);
    }

    const aviso = texto(pagamento?.avisoPagamento);
    if (aviso) {
      const limpo = aviso
        .replace(/^Aguardando\s+/i, "")
        .replace(/[.]$/, "")
        .trim();
  ...
  function motivosAmigaveis(pagamento) {
    ...
    const faltantes = faltantesDoPagamento(pagamento);
    const motivos = [];
    ...
    faltantes.forEach(faltante => {
      ...
      if (chave.includes("DEFINICAO DA LATERAL")) {
        adicionar("Falta definir quem fez a LATERAL");
        return;
      }
      if (chave.includes("DEFINICAO DO BOJO")) {
        adicionar("Falta definir quem fez o BOJO");
        return;
      }
      if (chave.includes("LATERAL")) {
        adicionar(referencia ? `Falta valor da LATERAL — Ref. ${referencia}` : "Falta valor da LATERAL");
        return;
      }
      if (chave.includes("ENCAPAR BOJO") || chave.includes("BOJO")) {
        adicionar(referencia ? `Falta valor do ENCAPAR BOJO — Ref. ${referencia}` : "Falta valor do ENCAPAR BOJO");
        return;
      }
  ...
  function atualizarSituacao(item, motivos) {
    ...
    const candidatos = [...item.querySelectorAll("span, strong, div")]
  ...
  async function processar() {
    ...
    const itens = [...modal.querySelectorAll(".corponu-pendencia-item")];
    await Promise.all(itens.map(processarItem));
  }
  ...
  function iniciar() {
    ...
    document.addEventListener("click", event => {
      ...
      if (chave.includes("PENDENCIA") || chave.includes("ATUALIZAR LISTA")) {
        [0, 80, 220, 500].forEach(atraso => window.setTimeout(() => {
          observarModal();
          agendar();
        }, atraso));
      }

...
if (parametros.get("v2write") !== "ordens-manejo") {
  parametros.set("v2write", "ordens-manejo");
  precisaRedirecionar = true;
}
if (precisaRedirecionar) {
  window.location.replace(`${window.location.pathname}?${parametros.toString()}`);
  throw new Error("REDIRECIONANDO_PARA_V2_ESCRITA_CONTROLADA");
}
...
async function carregarBridge() {
  ...
  informar("Login confirmado. Abrindo escrita controlada de Ordens e Manejo…");
...
form.addEventListener("submit", async event => {
  ...
  try {
    ...
    console.error("[V2 Escrita Controlada] Falha de autenticação.", error);
  } finally {
    senha.value = "";

...
try {
  $listener.Start()
  $url = "http://127.0.0.1:$Port/firebase-v2-fechamento-controlado/"

  Write-Host ""
  Write-Host "============================================================"
  Write-Host " CORPO NU FLOW V2 - FIREBASE REAL"
  Write-Host " ESCRITA CONTROLADA - FECHAMENTO DE PAGAMENTOS"
  Write-Host "============================================================"
  Write-Host ""
  Write-Host "Servidor local iniciado em: $url"
  Write-Host "Cada fechamento exige digitar GRAVAR."
  Write-Host "Somente entregasPagamento pode receber a transacao V2."
  Write-Host "Quitacao e demais areas permanecem bloqueadas."
  ...
  while ($true) {
    ...
    try {
      ...
      if ($requestPath -eq "/") { $requestPath = "/firebase-v2-fechamento-controlado/" }
  ...
  Write-Host ""
  Write-Host "Nao foi possivel iniciar a validacao do Fechamento." -ForegroundColor Red
  Write-Host $_.Exception.Message -ForegroundColor Red
  Write-Host ""
  Write-Host "Se a porta 8769 ja estiver em uso, feche outra validacao aberta e tente novamente."

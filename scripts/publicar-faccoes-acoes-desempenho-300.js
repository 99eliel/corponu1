...
chegada = chegada.split("onclick=\"registrarChegadaMovimentacao('${mov.id}')\"").join("type=\"button\" data-faccoes-acao=\"chegada\" data-movimentacao-id=\"${mov.id}\"");
...
if (render.includes('onclick="biparMovimentacao') || render.includes('onclick="excluirMovimentacao')) throw new Error('Bipar/Excluir ainda dependem de onclick.');
...
for (const esperado of ['const movimentacoesPorId = new Map();','function instalarControladorAcoesFaccoes()','data-faccoes-acao="chegada"','data-faccoes-acao="bipar"','data-faccoes-acao="excluir"','reconstruirIndiceMovimentacoes();','renderPaginaAtiva();']) {
  if (!source.includes(esperado)) throw new Error(`Invariante ausente: ${esperado}`);
}
fs.writeFileSync(arquivo, source, 'utf8');
console.log('Facções 300 aplicada com sucesso.');
